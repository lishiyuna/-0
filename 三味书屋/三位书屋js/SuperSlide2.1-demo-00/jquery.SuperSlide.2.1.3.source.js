...
(function($){
	$.fn.slide=function(options){
		...
		delayTime:500, // 效果速度
		interTime:2500, // 自动运行时间间隔
		triggerTime:150, // 触发时间
		defaultIndex: 0, // 默认分页索引，0是第一页，deaultIndex:1 相当于默认显示第二页
		titCell:".hd li", // 导航元素对象（鼠标的触发元素对象）
		mainCell:".bd", // 切换元素的包裹层对象
		targetCell:null, // 切换元素对象，处理内容切换元素非包裹状态
		...
		opp: false,  // 默认反方向运动
		pnLoop: true,  // 前后按钮循环
		easing: "swing", // 缓动效果 速度曲线
		startFun:null, // 每次切换效果开始时执行函数
		endFun:null,// 	每次切换效果结束时执行函数，用法和startFun一致
		switchLoad:null, // 内容切换加载，暂时只支持图片，必须配合后台程序使用。

		playStateCell:".playState", // 分页状态对象，用于显示分页状态，例如：2/3
